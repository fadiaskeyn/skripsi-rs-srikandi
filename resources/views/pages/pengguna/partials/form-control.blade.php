...
<div class="md:flex md:items-center mb-6 gap-2">
   <x-forms.select name="role" label="Role Pengguna" :options="$roles" />
</div>
<div class="md:flex md:items-center mb-6 gap-2">
    <x-forms.input id="username" type="text" value="{{ $user->username }}" name="username" label="Username" />
</div>
<div class="md:flex md:items-center mb-6 gap-2">
    <x-forms.input id="password" type="password" value="" name="password" label="Password" />
</div>
<div class="bg-white flex gap-5 ">
    ...
    <a href="{{ route('admin.user.index') }}" class="inline-flex items-center px-4 py-2 bg-theme-border-sidebar hover:bg-gray-700 text-white text-sm md:text-left font-medium">
        {{-- <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
